...
sub format_as_google_transl_return{
	...
	foreach my $transl_lang (@$transl_langs){
...
sub format_as_track_list_graph_axis{
	...
	foreach	my $graph_user (@$graph_users){
...
sub format_as_localized{
  ...
  $value = [];
	push @$value, lc $_->[0] foreach(@$codes);
  return join(' ',@$value);
}
...
sub format_as_onmarket_select
{
    ...
    foreach my $d (@$distri) { push @$rows, [$d->[0],$d->[1]]; }
...
# marking for saving the cat's children
	 foreach my $child(@{$tmp->{$cid}->{'children'}}){
 		 $tmp->{$child}->{'mark'} = 1;
	 }
...
sub format_as_remove_control_ASCII_chars {
	...
	if ($value) {
		...
		foreach my $symbol (@$bad_ASCII_symbols) {
			$value =~ s/$symbol//gs;
		}
	}
...
# &log_printf ('dropdown!!!'.&Dumper ( $value ) );
	...
	if (!$already_formed_list){
		...
		if ($atoms->{$call->{'class'}}->{$call->{'name'}}->{$field.'_related_dropdown_owned'} ne 'UNDEF'){
			my $selected=undef;
			foreach my $row(@$global_related_dropdown_rows) {
				if ($row->[0]==$value) {
					$selected=$row->[1];
					last;
				}
...
sub format_as_market_state {
	...
	my $suppliers_array;
	my $suppliers_string;
	push @$suppliers_array, $_->[0] foreach (@$suppliers);
...
sub format_as_language_flag {
    ...
    foreach (my $cnt; $cnt < ($#$codes + 1) - length($langs_string); $cnt++) {
...
sub format_as_mail_dispatch_groups
{
 ...
 foreach my $group_name(@groups_names){
	 my $checked = "";
...
#build unic emails
 ...
 foreach my $email(keys %$unic_hash){
	 $value .= $email.", "; $cnt++;
	 if($cnt == 2){ $value .= "\r\n"; $cnt=0;}
 }
 $value =~ s/(.+),\s$/$1/;
...
# $value = "<tr  width=100% colspan=3 bgcolor=white align=right>";
	$value = '';
	my $cnt = 0;
	foreach my $gallery_pic (@$gallery_pics) {
		$cnt ++;
...
and ". $call->{'call_params'}->{'search_changetype_prepared'}." and ej.score = 1 group by pd.langid");
	...
	foreach my $description_detail (@$description_details) {
		$descr_hash->{$description_detail->[2]} += $description_detail->[1];
	}

	$hash->{'summary_description_details'} = '';

	foreach my $key (sort keys %$descr_hash) {
...
sub format_as_supplier_contact {
	...
	foreach my $row(@$markets) {
		$markets_hash->{$row->[0]} = 1;
	}
	my $country_name;
	my $cntr = {};

	foreach my $country (@$countries_c, @$countries_u) {
		# do not duplicate
...
sub format_as_radio_buttons{
	...
	my $out_str="<table>";
	my $checked='';
	foreach my $radio_value(@radio_values_sorted){
	...
	if(!( -e $csv_file)){
		...
		if($hin{'feed_config_commands'} eq 'reupload_price_feed' or 1){# currently is not checked
			...
			if(!$value or $value eq '0' and $autodetect_keys and $hash->{'is_first_header'}){# try to auto detect column
				...
				foreach my $row(@$rows){
					my $a=1;
					foreach my $field_part(@field_parts){
...
sub format_as_track_list_restricted_cols{
	...
	my $i=1;
	foreach my $col(@$restricted_cols){
