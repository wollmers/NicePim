...
sub end_document {
	my $self=shift;
	for my $rep (keys(%{$self->{repeatTags}})) {
	...
	for my $tag_path (keys(%{$self->{repeatTags}})) {
		my $is_rep_parent=undef;
		my $is_csv_tag=undef;
		for my $rep_parent (keys %{$self->{rep_parents}}) { 
			if($tag_path=~/^\Q$rep_parent\E/){
				$is_rep_parent = 1;
			};
		}
		next if $is_rep_parent;
		for my $csv_path (keys %{$self->{csv_paths}}) { 
	...
	my @header_arr;
	for my $header (keys(%{$self->{csvHeader}})) {
	...
	$cnt = 0;
	for my $header(@header_arr){
		for my $attr (keys(%{$header->{'attrs'}})) {
