	...
	if ($langid && $langid != 1) {
		foreach my $feat (@$feats) {
			if (scalar(grep(/^$feat->[21]$/, ('dropdown','multi_dropdown','text','textarea','')))) {
				...
				foreach	my $multi_feat (@multi_feats) {
...
sub _is_a_standard_value {
  my ($value, $value_wo_unit, $langid) = @_;
	...
	if ($standard_values_local == undef) {
		...
		foreach (@$standard_values_local_from_database) {
			push @{$standard_values_local->{$_->[1]}}, $_->[0];
		}
	}

	# check ENglish values only
  foreach (@$standard_values_EN) {
#		log_printf("EN: ".$_);
  ...
  foreach (@$standard_values_specific_lang) {
...
sub load_data2globals
{
 if(!defined $category_features){
  ...
  foreach my $row (@$raw_category_features){
	...
	foreach my $row (@$raw_category_feature_groups){
...
sub conform_product_feature_catid {
	...
	foreach my $row(@$product_features) {
...
foreach my $row(@$assigned_src){
# foreach entry we are checking if there exists $dst_id feature in category features

my $dst_is_assigned = 0;
my $cat_feat_id; # of dst category feature

  foreach my $row1(@$assigned_dst){
 ...
 if(defined $prefs&&defined $prefs->[0]){
  ...
  # each symbol should be unique!
	...
	if (defined @$suppliers) {
	    my @s = @$suppliers;
	    foreach (0 .. $#s) { $condition.=" supplier_id='".$s[$_]."' or " }
	...
	unless ( $shadow->{'category_feature'} ){
		...
		foreach my $cat_feat(@$cat_feat_ref){
...
#		foreach my $product_feat(@$product_feat_ref){
...
sub send_report {
	...
	foreach (sort keys %{$missing->{'distributor'}}) {
	...
	foreach my $feature_id (keys %{$missing->{'feature_value_mapping'}}) {
	...
	foreach $row (@{$prefs->{'ignored_products_list'}}) {
		push @{$ignored_products_grouped->{$row->{'reason'}}->{$row->{'ocat'}}}, $row;
	}
	
	foreach my $reason (keys %$ignored_products_grouped) {
		$ignored_products .= "  ".$reason.":\n";
		foreach my $ocat (sort keys %{$ignored_products_grouped->{$reason}}) {
			$ignored_products .= "        ".$ocat.":\n";
			foreach $row (@{$ignored_products_grouped->{$reason}->{$ocat}}) {
...
foreach my $symbol(keys %{$prefs->{'category_map'}}){
...
foreach my $symbol(keys %{$prefs->{'feature_map'}}){
	foreach my $catid(keys %{$prefs->{'feature_map'}->{$symbol}}){
	 foreach my $hash($prefs->{'feature_map'}->{$symbol}->{$catid}->{'feature_id'}){
			if($hash->{'feature_id'}){
...
foreach my $item(keys %$product_ins){
...
sub add_symbols_to_mapping {
	...
	foreach $item (@$list) {
# &log_printf($item);
		foreach $symbol (keys %{$missing->{$item}}) {
...
where ".$where);
 ...
 foreach my $supplier_id(keys %$mapped){
  foreach my $prod_id(keys %{$mapped->{$supplier_id}}){
...
sub smart_update_feature_local {
  ...
  foreach my $category_feature_id(keys %$values){
		my $value;
