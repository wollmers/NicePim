		...
		if (($report_type==2) || ($hash->{'scat'}!=1)) {
		  ...
		  my @catids;
			sub get_catids {
				my ($id) = @_;
				push @catids,$id;
				foreach my $subid (@{$cats_by_owner->{$id}}) {
					&get_catids($subid);
				}
			}
